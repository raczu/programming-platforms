...
class ProblemTest {
    ...
    void Problem_whenNumberOfItemsIsNegative_ThrowIllegalArgumentException() {
        assertThrows(IllegalArgumentException.class, () -> new Problem(-1, 10, 1, 10));
    }
    ...
    void Problem_whenNumberOfItemsIsCorrect_GenerateWantedNumberOfItems() {
        ...
        for (int size : sizes) {
            Problem problem = new Problem(size, 10, 1, 10);
            assertEquals(size, problem.items.size());
        }
    }

    @Test
    void Problem_whenBoundsSpecified_GenerateItemsWithinBounds() {
        Problem problem = new Problem(100, 10, 1, 10);
